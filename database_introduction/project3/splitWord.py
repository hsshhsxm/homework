...
class splitWord:
    def __init__(self, url, usrname, usrpwd, dbname, logNo):
        try:
            ...
            if not os.path.exists(folder):
                os.makedirs(folder)
            fileName = "log/splitWordLog-" + str(logNo) + ".log"
    ...
    #第n行分词
    def split(self, No):
        cmd = "SELECT * FROM project.travel_poi_checkin_weibos_suzhou LIMIT " + str(No-1) + ",1;"
        self.cursor.execute(cmd)
...
class myThread(threading.Thread):
    def __init__(self, threadID, start, end):
        ...
        self.start = start
        self.end = end
        self.db = splitWord("localhost", "hs", "hs", "project", threadID)
...
if __name__ == "__main__":
    db = splitWord("localhost", "hs", "hs", "project", 0)
    num =db.getNum()
    threadNum = 10
    num_10 = num / threadNum
    threads = []
    for i in range(0, threadNum):
        thread = myThread(i+1, num_10 * i + 1, num_10 * i + num_10 + 1)
